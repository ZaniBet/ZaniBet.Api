...
router.post('/grilles/tournament', passport.authenticate('bearer', { session: false }), function(req, res){
  ...
  if (currentUser.locale != null && currentUser.locale == "fr"){
    errLocalized = {
      "internal": new CustomError(500,1, "Erreur Interne", ""),
      "grid_max_play": new CustomError(500, -1, "Limite Atteinte", "Vous avez atteind la limite de grilles jouables pour ce ticket."),
      "insufficient_zanihash": new CustomError(500,-1, "Solde Insuffisant", "Vous n'avez pas assé de ZaniHash pour participer à ce tournoi ! Rejoignez le programme ZaniAnalytics pour recevoir quotidiennement des ZaniHash."),
    };
  } else {
    errLocalized = {
      "internal": new CustomError(500,1, "Internal Error", ""),
    ...
    function(gameticket, done){
      ...
      .then(function(transaction){
        ...
        if (result.nModified == 1){
          ...
          return User.updateOne({ _id: currentUser._id, pendingTransactions: { $ne: _transaction._id }, "wallet.zaniHash": { $gt: gameticket.tournament.playCost } }, { $inc: { "wallet.zaniHash": _transaction.amount }, $push: { pendingTransactions: _transaction._id } }).exec();
        } else {
    ...
    */
    function(grille, done){
      GameTicket.findOne({ _id: grille.gameTicket }, function(err, gameticket){
        ...
        var pot = (tournament.playCost/process.env.ZH_TO_ZC).toFixed(0);
