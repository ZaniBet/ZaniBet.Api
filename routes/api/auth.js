...
/*router.get('/auth/resetPassword/:token', function(req, res){
...
// Send request from app for reset password
router.put('/auth/resetPassword', passport.authenticate('oauth2-client-password', { session: false }), function(req, res, next){

  var email = req.body.email;

  if (email == null || !validator.isEmail(email)){
    return res.status(500).json(new CustomError(50, "Error", "Vous devez indiquer l'adresse email utilisé lors de votre inscription."));
  }
    ...
    function(token, done) {
      User.findOne({ email: email }, function(err, user) {
        if (!user) {
          ...
          user.resetPasswordExpires = Date.now() + (1000 * 60 * 60 * 24); // 24 hour
