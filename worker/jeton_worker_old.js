  ...
  return new Promise(function(resolve, reject){
    ...
    async.waterfall([
      ...
      function(done){
        ...
        User.find({ jeton: { $lt: 2 }, $or: [{ lastFreeJeton: { $exists: false } }, { lastFreeJeton: { $lt: lastDay } }] }).toArray().then(function(users){
          console.log(users.length, "utilisateurs trouvés");
          return done(null, users);
        }, function(err){
          console.log("Impossible de trouver les utilisateurs devant recevoir des jetons.");
          return done(err);
        });
      },
      ...
      function(users, done){
        ...
        User.updateMany({ _id: { $in: userIdArr } }, { $set: { updatedAt: moment().utc().toDate(), lastFreeJeton: moment().utc().toDate() }, $inc: { jeton: 5 }  }, function(err, result){
          if (err){
      ...
      function(users, done){
        ...
        GCM.sendSingleMessage(tokenArr, "ZaniBet", "5 jetons viennent d'être crédités sur ton compte ZaniBet. Fais tes pronostics pour les matchs à venir !");
        return done(null);
      }
