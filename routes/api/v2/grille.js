...
router.get('/grilles', passport.authenticate('bearer', { session: false }), function(req, res){
  ...
  Grille.aggregate([
    ...
    .populate({
      path: 'gameTicket',
      select: '_id type name openDate limitDate resultDate thumbnail picture fixtures betsType competition pointsPerBet status matchDay jackpot maxNumberOfPlay numberOfGrillePlay',
      populate: {
        path: 'fixtures',
        select: 'date competition homeTeam awayTeam matchDay result status',
        populate: {
