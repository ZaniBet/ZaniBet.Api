...
router.get('/gametickets', passport.authenticate('bearer', { session: false }), function(req, res){
  ...
  if (ticketType == "SINGLE"){
    ...
    .select('_id type name openDate limitDate resultDate thumbnail fixtures betsType competition pointsPerBet status matchDay jeton maxNumberOfPlay numberOfGrillePlay')
    .populate({
      path: 'competition',
      select: 'logo name country'
    })
    .populate({
      path: 'fixtures',
      select: 'date homeTeam awayTeam result status odds',
      options: { sort: { 'date': 1 } },
      populate: {
        path: 'homeTeam awayTeam',
        select: 'name shortName logo country'
      }
    })
    .skip(page*limit)
    .limit(limit)
    ...
    if (filterCompetition != null && filterCompetition != ""){
      gameticketQuery.where('competition').eq(filterCompetition);
    }
  } else if (ticketType == "MATCHDAY"){
    gameticketQuery = GameTicket.find({ openDate: { $lt: currentDate }, limitDate: { $gt: currentDate }, status: 'open', active: true, type: ticketType })
    .select('_id type name openDate limitDate resultDate picture fixtures pointsPerBet status matchDay jackpot maxNumberOfPlay numberOfGrillePlay')
    /*.populate({
      path: 'competition',
      select: 'name'
    })*/
