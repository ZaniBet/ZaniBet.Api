...
var server = "wss://zhash1.zanibet.com:5141";
...
wsp.open().then(function(){
  MongoClient.connect(process.env.DB_URI, function(err, db) {
    ...
    console.log("-----> Start pool_worker.js <-----");
    poolJob(db).then(function(res){
      wsp.close();
      db.close();
      console.log("-----> Pool Worker Job Done <-----");
    }).catch(function(err){
      console.log(err);
      wsp.close();
      db.close();
  ...
  console.log("Impossible de se connecter au wss", err);
});
